...
def investor_flow(symbol: str, start_date: str = '', end_date: str = '') -> InvestorFlowResult:
    try:
        from pykrx import stock
        s_date, e_date = _get_dates(start_date, end_date)
        df = stock.get_market_trading_value_by_investor(s_date, e_date, symbol, etf=False, etn=False, elw=False)
        return {
            "symbol": symbol,
...
def market_investor_flow(date: str = '') -> InvestorFlowResult:
    try:
        ...
        # KOSPI
        df_kospi = stock.get_market_net_purchases_of_equities_by_investor(s_date, e_date, "KOSPI", "ALL")
        items_kospi = _records(df_kospi) if df_kospi is not None else []
        for item in items_kospi:
            item["market"] = "KOSPI"
            
        # KOSDAQ
        df_kosdaq = stock.get_market_net_purchases_of_equities_by_investor(s_date, e_date, "KOSDAQ", "ALL")
